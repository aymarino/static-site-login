# Deploys the 'authenticate-user' lambda
set -e
...
FUNCTION_NAME=authenticate-user-fn
...
CONFIG_FILE=$(mktemp)
echo $CONFIG \
  | jq '(.DistributionConfig.CacheBehaviors.Items[0].LambdaFunctionAssociations.Items[0].LambdaFunctionARN) |= "'"$NEW_FUNCTION_ARN"'"' \
